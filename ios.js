...
let currentSegment = 'all'; // 'all' | 'sales' | 'chengjiao' | 'offshelf'
...
function getStoredPin() {
    return safeGetItem('yc_ios_pin', '') || safeGetItem('real_estate_app_auth_pin', '');
}
...
function initAuth() {
    ...
    if (storedPin) {
        ...
        loadDatasets(storedPin).then(ok => {
            if (!ok) {
                if (overlay) overlay.style.display = 'flex';
...
async function checkAuth() {
    ...
    if (!val) {
        if (errorMsg) errorMsg.textContent = '請輸入通行密碼';
        return;
    }

    if (errorMsg) errorMsg.textContent = '🔓 正在驗證與解密資料庫...';
    const success = await loadDatasets(val);
    if (success) {
        safeSetItem('yc_ios_pin', val);
...
async function loadDatasets(pin) {
    const effPin = pin || getStoredPin();
    ...
    try {
        ...
        const [rawSales, rawChengjiao, rawOffshelf] = await Promise.all([
            ...
            fetchJsonWithRetry('offshelf_data.json')
        ]);
        ...
        if (sRes === null && cRes === null && oRes === null) {
            return false; // Decryption failed due to wrong password
        }
        ...
        allData = [...salesData, ...chengjiaoData, ...offshelfData];

        const countAll = document.getElementById('allSegCount');
        const countSales = document.getElementById('salesSegCount');
        const countSold = document.getElementById('soldSegCount');
        const countOffshelf = document.getElementById('offshelfSegCount');
        if (countAll) countAll.textContent = allData.length.toLocaleString();
        ...
        if (countOffshelf) countOffshelf.textContent = offshelfData.length.toLocaleString();
...
function refreshData() {
    ...
    loadDatasets().then((ok) => {
        setTimeout(() => icon.classList.remove('fa-spin'), 600);
        if (ok) showToast('✅ 資料庫已同步至最新');
        else showToast('⚠️ 同步失敗，請檢查密碼');
    });
}

// ─── 切換分段 (全部 / 銷售中 / 已成交 / 已下架) ───────────────────────────
function switchSegment(seg) {
    currentSegment = seg || 'all';
    const tabAll = document.getElementById('tabAllBtn');
    if (tabAll) tabAll.classList.toggle('active', currentSegment === 'all');
    document.getElementById('tabSalesBtn').classList.toggle('active', currentSegment === 'sales');
    document.getElementById('tabSoldBtn').classList.toggle('active', currentSegment === 'chengjiao');
    const offBtn = document.getElementById('tabOffshelfBtn');
    if (offBtn) offBtn.classList.toggle('active', currentSegment === 'offshelf');
    currentPage = 1;
    applyFilterAndRender();
}
...
function applyFilterAndRender() {
    let rawData = allData;
    if (currentSegment === 'sales') rawData = salesData;
    else if (currentSegment === 'chengjiao') rawData = chengjiaoData;
    else if (currentSegment === 'offshelf') rawData = offshelfData;
...
function sortData(list, sortType) {
    list.sort((a, b) => {
        ...
        if (sortType === 'price-asc') {
            const vA = (a.total_price && a.total_price > 0) ? a.total_price : Infinity;
            const vB = (b.total_price && b.total_price > 0) ? b.total_price : Infinity;
            return vA - vB;
        }
        ...
        if (sortType === 'unit-asc') {
            const vA = (a.unit_price && a.unit_price > 0) ? a.unit_price : Infinity;
            const vB = (b.unit_price && b.unit_price > 0) ? b.unit_price : Infinity;
            return vA - vB;
        }
        if (sortType === 'unit-desc') {
            const vA = (a.unit_price && a.unit_price > 0) ? a.unit_price : -Infinity;
            const vB = (b.unit_price && b.unit_price > 0) ? b.unit_price : -Infinity;
            return vB - vA;
        }
        if (sortType === 'area-desc') {
            ...
            const vA = (pA > 0) ? pA : -Infinity;
            const vB = (pB > 0) ? pB : -Infinity;
            return vB - vA;
        }
        if (sortType === 'area-asc') {
            const pA = a.area_ping || a.build_ping || a.land_ping || 0;
            const pB = b.area_ping || b.build_ping || b.land_ping || 0;
            const vA = (pA > 0) ? pA : Infinity;
            const vB = (pB > 0) ? pB : Infinity;
...
function renderCardList() {
    ...
    pageItems.forEach((item) => {
        ...
        const dateBadge = item.date ? `<span class="card-date-badge"><i class="fa-regular fa-calendar-days"></i> ${item.date}</span>` : '';
        let statusBadge = '';
        if (item.status === '已售出' || item.status === '成交') {
            statusBadge = `<span class="card-status-badge card-status-sold"><i class="fa-solid fa-handshake"></i> 已售出</span>`;
        } else if (item.status === '已下架') {
            statusBadge = `<span class="card-status-badge card-status-off"><i class="fa-solid fa-ban"></i> 已下架</span>`;
        } else {
            statusBadge = `<span class="card-status-badge card-status-sales"><i class="fa-solid fa-house-circle-check"></i> 銷售中</span>`;
        }
...
function openDetailSheet(encodedItem) {
    ...
    // 歷次委託與改價軌跡渲染
    const histSection = document.getElementById('sheetHistorySection');
    const histTimeline = document.getElementById('sheetHistoryTimeline');
    if (histSection && histTimeline) {
        if (item.history && Array.isArray(item.history) && item.history.length > 0) {
            histSection.style.display = 'block';
            let histHtml = '';
            // Newest on top
            const revHist = item.history.slice().reverse();
            revHist.forEach((h, idx) => {
                const isCurrent = idx === 0;
                histHtml += `
                    <div class="history-timeline-item ${isCurrent ? 'current' : ''}">
                        <div class="history-timeline-dot"></div>
                        <div class="history-timeline-header">
                            <span class="history-timeline-date"><i class="fa-regular fa-calendar"></i> ${h.date || '--'}</span>
                            <span class="history-timeline-price">${h.total_price ? h.total_price.toLocaleString() + ' 萬' : ''}</span>
                        </div>
                        ${h.name ? `<div class="history-timeline-name"><i class="fa-solid fa-tag"></i> 案名：${h.name}</div>` : ''}
                        <div class="history-timeline-event">${h.event || '資料異動更正'}</div>
                        ${h.agent ? `<div class="history-timeline-meta"><i class="fa-solid fa-user-tie"></i> ${h.agent} ${h.store_name ? '· ' + h.store_name : ''}</div>` : ''}
                    </div>
                `;
            });
            histTimeline.innerHTML = histHtml;
        } else {
