...
let currentSegment = 'sales'; // 'sales' | 'chengjiao' | 'offshelf'
...
function switchSegment(seg) {
    ...
    if (offBtn) offBtn.classList.toggle('active', seg === 'offshelf');
    currentPage = 1;
    applyFilterAndRender();
}
...
function applyFilterAndRender() {
    ...
    if (currentSegment === 'chengjiao') rawData = chengjiaoData;
...
function renderCardList() {
    ...
    pageItems.forEach((item) => {
        ...
        const statusBadge = (item.status === '已下架' || currentSegment === 'offshelf')
            ? `<span class="card-offshelf-badge"><i class="fa-solid fa-ban"></i> 已下架</span>`
            : '';
